...
| Toggle „Quellen-Übersetzung" in falschem Kontext (zentrale Toolbar) | done | Verschoben in Quellen-Panel-Header |
| Schichten-Toggle wirkt nicht, wenn Edition-Slot geschlossen | done | `ensurePoolVisible('edition')` |
| Quellen-nhd-Toggle wirkt nicht, wenn Quellen-Slot geschlossen | done | `ensurePoolVisible('sources')` |
| Quellen-/Psalter-Filter untereinander statt nebeneinander | done | `flex-direction: row` + `.source-filter-group` |
| Redundante Zeilen im Quellen-Header (Subheader + Filterzeile) | done | Merge zu einer Zeile, Aktion via `margin-left:auto` |
| Toolbar-Gruppenlabels „SCHICHTEN"/„ANZEIGE" verschwenden Platz | done | Entfernt, Padding/Font reduziert |
| Fünf `<l>`-Zeilen-Korrekturen greifen nicht (Fließtext-Replace) | done | Patterns für beide Kontexte in `PFEIFER_CORRECTIONS` |
| Cross-Verse-Drift: „erlaubte es" bei V3 statt V1–2 | done | `redistribute_crossverse_nhd()` in `build_tei.py` |
| Psalm-Nav 1/3/50/100/150 suggeriert begehbare Psalmen | done | `.disabled`-Class + `cursor:not-allowed` |
| Diagnose-Konsole meldet Glossenzahl 13/14 als Problem | done | Erwartung auf 13 korrigiert |
